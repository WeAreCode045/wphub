...
Deno.serve(async (req) => {
    try {
        ...
        console.log('[activatePlugin] === START ===');
        console.log('[activatePlugin] Site ID:', site_id);
        console.log('[activatePlugin] Plugin ID:', plugin_id);
        ...
        console.log('[activatePlugin] Plugin:', plugin.name, '(slug:', plugin.slug, ')');

        // Call connector endpoint
        const connectorUrl = `${site.url}/wp-json/wphub/v1/activatePlugin`;
        console.log('[activatePlugin] Calling connector:', connectorUrl);
        ...
        if (!response.ok) {
            ...
            console.error('[activatePlugin] Connector error:', errorText);
        ...
        console.log('[activatePlugin] Connector response:', result);
        ...
        if (result.success) {
            ...
            if (pluginIndex >= 0) {
                currentPlugins[pluginIndex].is_activated = 1;
            }
            ...
            console.log('[activatePlugin] ✅ Success - plugin activated');
        }

        console.log('[activatePlugin] === END ===');

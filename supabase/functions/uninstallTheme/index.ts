...
import { UninstallThemeRequestSchema, z } from '../_shared/schemas.ts';
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    try {
      ...
      body = UninstallThemeRequestSchema.parse(parsed);
    } catch (parseError) {
      console.error('[uninstallTheme] Validation error:', parseError);
    ...
    const { site_id, theme_slug, theme_id } = body;

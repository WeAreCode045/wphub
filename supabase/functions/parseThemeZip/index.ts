...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log('[parseThemeZip] All files in ZIP:', Object.keys(zip.files).slice(0, 20)); // Log first 20 files
    
    for (const [path, file] of Object.entries(zip.files)) {
      const fileName = path.toLowerCase();
      
      // Look for screenshot anywhere in the theme, not just at root level
      if ((fileName.includes('screenshot.png') || fileName.includes('screenshot.jpg') || fileName.includes('screenshot.jpeg')) && !file.dir) {
        try {
          console.log('[parseThemeZip] Found screenshot:', path);
        ...
        break; // Stop after finding first screenshot
      }
    }

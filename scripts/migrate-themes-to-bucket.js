...
async function uploadToThemesBucket(filePath, fileName) {
  try {
...
async function migrateThemesToBucket() {
  console.log('Starting migration of theme files to Themes bucket...\n');

  try {
    // Get all themes with versions
    ...
    const tempDir = path.join(__dirname, '..', 'temp_themes_migration');
    if (!fs.existsSync(tempDir)) {
      fs.mkdirSync(tempDir, { recursive: true });
    }

    for (const theme of themes) {
      console.log(`Processing theme: ${theme.name} (ID: ${theme.id})`);

      if (!theme.versions || !Array.isArray(theme.versions)) {
      ...
      for (const version of theme.versions) {
        ...
        if (version.download_url.includes('qtrypzzcjebvfcihiynt.supabase.co')) {
          ...
          try {
            ...
            // Upload to Themes bucket
            console.log(`    Uploading to Themes bucket...`);
            const newUrl = await uploadToThemesBucket(tempFilePath, fileName);
      ...
      // Update theme if there were changes
      if (hasChanges) {
        ...
        console.log(`  No changes needed for theme ${theme.name}`);
      }

      processedCount++;
      console.log(`  Progress: ${processedCount}/${themes.length} themes processed\n`);
    }
    ...
    console.log('\nMigration completed!');
    console.log(`Total themes processed: ${processedCount}`);
